...
describe('cookies return the expected consent', () => {
	...
	beforeEach(() => {
		...
		(getCookie_ as jest.Mock).mockImplementation(
			({ name }: { name: string }) => {
				if (name === 'gu.mock') {
					return mockContains;
				} else {
		...
		(setCookie_ as jest.Mock).mockImplementation(
	...
	test('Targeted advertising get cookie returns null when canTarget is false', async () => {
		...
		const cookieValue = await cmpGetCookie({
	...
	test('Targeted advertising can set and get cookies when canTarget is true', async () => {
		...
		mockOnConsentChange(consentState);
		const cookieValueDefault = await cmpGetCookie({
			useCase: 'Targeted advertising',
			name: 'gu.mock',
		});

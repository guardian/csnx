...
export const cmpGetCookie = async ({
	useCase,
	name,
	shouldMemoize,
}: {
	...
	return cmpGetCookieWithConsentState({
		useCase,
		consentState,
		name,
		shouldMemoize,
...
export const cmpGetCookieWithConsentState = ({
	useCase,
	consentState,
	name,
	shouldMemoize,
	...
	if (hasConsentForUseCaseWithConsentState(useCase, consentState)) {
		...
		console.error(
			'cmp',
			`Cannot get cookie ${name} due to missing consent for use-case ${useCase}`,
		);
		return null;
	}
};

export const cmpSetCookie = async ({
	useCase,
	name,
	value,
	daysToLive,
	...
	const consentState = await onConsent();
	return cmpSetCookieWithConsentState({
		useCase,
		consentState,
		name,
		value,
...
export const cmpSetCookieWithConsentState = ({
	useCase,
	consentState,
	name,
	value,
	...
	if (hasConsentForUseCaseWithConsentState(useCase, consentState)) {
		...
		console.error(
			'cmp',
			`Cannot set cookie ${name} due to missing consent for use-case ${useCase}`,
		);
	}
};

export const cmpSetSessionCookie = async ({
	useCase,
	name,
	value,
}: {
	...
	const consentState = await onConsent();
	return cmpSetSessionCookieWithConsentState({
		useCase,
		consentState,
		name,
		value,
...
export const cmpSetSessionCookieWithConsentState = async ({
	useCase,
	consentState,
	name,
	value,

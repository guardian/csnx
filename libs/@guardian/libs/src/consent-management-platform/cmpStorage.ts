...
export const storageOptions = ['localStorage', 'sessionStorage'] as const;
...
class StorageFactory {
	...
	getWithConsentState(
		...
		if (hasConsentForUseCaseWithConsentState(useCase, consentState)) {
			...
			console.error(
				'cmp',
				`Cannot get local storage item ${key} due to missing consent for use-case ${useCase}`,
			);
			return null;
		}
	}
	...
	setWithConsentState(
		...
		if (hasConsentForUseCaseWithConsentState(useCase, consentState)) {
			switch (this.#storageHandler) {
				case 'localStorage':
					return libsStorage.local.set(key, value, expires);
				case 'sessionStorage':
					return libsStorage.session.set(key, value, expires);
			}
		} else {
			console.error(
				'cmp',
				`Cannot set local storage item ${key} due to missing consent for use-case ${useCase}`,
			);
		}
	}
	...
	setRawWithConsentState(
		...
		if (hasConsentForUseCaseWithConsentState(useCase, consentState)) {
			switch (this.#storageHandler) {
				case 'localStorage':
					return libsStorage.local.setRaw(key, value);
				case 'sessionStorage':
					return libsStorage.session.setRaw(key, value);
			}
		} else {
			console.error(
				'cmp',

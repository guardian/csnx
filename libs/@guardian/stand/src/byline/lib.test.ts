...
describe('hasHitContributorLimit', () => {
	test('returns false when the limit does not exist', () => {
	...
	test('returns false when there are fewer nodes than the limit', () => {
	...
	test('returns true when there are as many nodes as the limit', () => {
		const testDoc = bylineEditorSchema.nodes.doc.create({}, [
			bylineEditorSchema.nodes.chip.create({ label: 'Test Chip 1' }),
			bylineEditorSchema.text('Test'),
			bylineEditorSchema.nodes.chip.create({ label: 'Test Chip 2' }),
	...
	test('returns true when there are more nodes than the limit', () => {
		const testDoc = bylineEditorSchema.nodes.doc.create({}, [
			bylineEditorSchema.nodes.chip.create({ label: 'Test Chip 1' }),
			bylineEditorSchema.text('Test'),
			bylineEditorSchema.nodes.chip.create({ label: 'Test Chip 2' }),
